...
communication requirements prerequisites.
    
--------------------------- REVISION HISTORY ------------------------------
 rev1: 18/05/2023 - Initial version
 rev2: 19/05/2023 - destructor code updated to fix issue with re-connection
 rev3: 05/07/2023 - Encrypted pass key and fix issue in BIO wrapper.
...
BIOWrapper::BIOWrapper(CommConfig *InterfaceConfig)
{
    ...
    m_InterfaceConfig = InterfaceConfig;

    /*Secure communication is disabled*/
    if(!m_InterfaceConfig->SecureConfigParam.IsEnable)
    {
        /*Role is SERVER*/
        if(m_InterfaceConfig->RoleIs == SERVER)
        {
...
int BIOWrapper::BIORead(uint8_t * const buff, const int size)
{
    ...
    if(CommStatusResult >= STATUS_CONNECTION_DEFAULT)
    {
        ...
        if(!m_InterfaceConfig->SecureConfigParam.IsEnable)
        {
            ...
            if(!m_InterfaceConfig->IsBlocking)
            {
                ...
                /* Check read retry count if its max then update return value as failure */
                if (read_retry_count == MAX_READ_RETRY_COUNT)
				{
					/* Update read status to failure */
					ReadStat=-1;
				}
            }
        ...
        else
        {
            ...
            if(!m_InterfaceConfig->IsBlocking)
            {
                ...
                /* check status of read function, if no data read then increment read counter */
                if (ReadStat == 0 ) { read_retry_count ++; }
...
******************************************************************************/
void BIOWrapper::IsConfigValid(CommConfig *InterfaceConfigVal)
{
    /*Check valid Role is configured*/
    if((InterfaceConfigVal->RoleIs < SERVER) || (InterfaceConfigVal->RoleIs > CLIENT))
    {
        throw std::invalid_argument("N:Invalid Role");
    }
    ...
    {
        throw std::invalid_argument("N:Invalid IP&Port");
    }

    if (InterfaceConfigVal->SecureConfigParam.IsEnable)
    {
		...
		{
			throw std::invalid_argument("N:Invalid PeerName");
		}
		...
		{
			throw std::invalid_argument("N:No Self Cert");
		}
		...
		{
			throw std::invalid_argument("N:No Private Key");
		}
		...
		{
			throw std::invalid_argument("N:Invalid C.Suite");
		}
		...
		{
			throw std::invalid_argument("N:Invalid TLS Ver.");
		}
    }
}
...
void BIOWrapper::NonSecureServerConfiguration()
{
    ...
    comm_bio = BIO_new_accept(m_InterfaceConfig->DestinationIPandPort);
    if (comm_bio == nullptr)
    {
        throw std::runtime_error("R:BIO_new_accept");
    }
    ...
    if(!m_InterfaceConfig->IsBlocking)
    {
        if (BIO_set_nbio(comm_bio, 1) <= 0)
        {
            ...
            throw std::runtime_error("R:BIO_set_nbio");
        }
    }
    
	// Following code line is workaround to enable reconnection.
	// Even after closing all socket connection after failure, during restart accept operation on same port fails.
	// This may be related to Openssl issue. not yet confirmed.
    BIO_set_bind_mode(comm_bio, BIO_BIND_REUSEADDR);
    ...
    if (BIO_do_accept(comm_bio) <= 0)
    {
    	/* Close Connection */
         ...
         throw std::runtime_error("R:BIO_do_accept");
    }
    
    std::cout << "server started on " << m_InterfaceConfig->DestinationIPandPort<<std::endl;
...
void BIOWrapper::NonSecureClientConfiguration()
{
    ...
    comm_bio = BIO_new_connect(m_InterfaceConfig->DestinationIPandPort);
        ...
        while(Retry)
        {
            ...
            if (BIO_do_connect(comm_bio) <= 0)
            {
                ...
                else
                {
                    ...
                    throw std::runtime_error("R:BIO_do_connect");
                }
            }
...
void BIOWrapper::SecureConfigurationSetup()
{
    ...
    if (ctx == nullptr)
    {
        throw std::runtime_error("N:SSL_CTX_new");
    }

    /*Set protocol version to be used*/
    RetVal = SSL_CTX_set_min_proto_version(ctx, m_InterfaceConfig->SecureConfigParam.TLSVersionIs);
    if(RetVal == 0)
    {
        throw std::runtime_error("N:SSL Prot Version");
    }

    /*Select ciphersuite to be used*/
    RetVal = SSL_CTX_set_ciphersuites(ctx,m_InterfaceConfig->SecureConfigParam.CipherSuite);
    if(RetVal == 0)
    {
        throw std::runtime_error("N:Set Ciphersuites");
    }

    /*Load the certificates and private keys into the SSL object*/
    if (SSL_CTX_use_certificate_file(ctx, m_InterfaceConfig->SecureConfigParam.SelfCertificate, SSL_FILETYPE_PEM) <= 0)
    {
        throw std::runtime_error("N:Certificate File");
    }

    
    std::ifstream PassKeyStream(m_InterfaceConfig->SecureConfigParam.PassKey);
    std::stringstream SecurePassKey;
    SecurePassKey << PassKeyStream.rdbuf();

    EncryptDecryptData encd;
    std::string EncryptPassKey = encd.DecryptString(SecurePassKey.str());

    /*Set the default password callback called when loading/storing a PEM certificate with encryption*/
    SSL_CTX_set_default_passwd_cb_userdata(ctx,(void*) EncryptPassKey.c_str());

    if (SSL_CTX_use_PrivateKey_file(ctx, m_InterfaceConfig->SecureConfigParam.SelfPrivateKey, SSL_FILETYPE_PEM) <= 0)
    {
        throw std::runtime_error("N:PrivateKey");
    }
    /*Set trusted certificate*/
    RetVal = SSL_CTX_load_verify_locations(ctx,m_InterfaceConfig->SecureConfigParam.CACertificate ,NULL);
    if(RetVal == 0)
    {
        throw std::runtime_error("N:Verify Locations");
    }
...
void BIOWrapper::SecureServerConfiguration()
{
	bool Retry = true;
	 /*Accept new connection for data transfer using BIO routine*/
    bio = BIO_new_accept(m_InterfaceConfig->DestinationIPandPort);
    if (bio == nullptr)
    {
	...
	std::cout << "Server started on " << m_InterfaceConfig->DestinationIPandPort<<"\n"<<std::endl;
	...
	if (connection_bio == nullptr)
	{
		...
		throw std::runtime_error("R:BIO_pop");
	}
	/*Create SSL BIO*/
	...
	if (comm_bio == nullptr)
	{
		...
		throw std::runtime_error("R:BIO_new_ssl");
	}
	...
	if (ssl == nullptr)
	{
		...
		throw std::runtime_error("R:BIO_get_ssl");
	}
		...
		while(Retry)
		{
			...
			if (BIO_do_handshake(comm_bio)<= 0)
			{
				...
				else
				{
					...
					throw std::runtime_error("R:BIO_do_handshake");
				}
			}
	...
	if (err != X509_V_OK)
	{
		...
		throw std::runtime_error("R:Sever Cert Err");
	}
	...
	if (cert == nullptr)
	{
		...
		throw std::runtime_error("R:No Peer Cert");
	}
	
	/*Check whether a certificate matches a given hostname, email address, or IP address*/
	if (X509_check_host(cert, m_InterfaceConfig->SecureConfigParam.ExpectedPeerName.data() , m_InterfaceConfig->SecureConfigParam.ExpectedPeerName.size(), 0, nullptr) != 1)
	{
    	/* Close Connection */
    	CloseConnection();
    	
		throw std::runtime_error("R:Peer Name Err");
	}
...
void BIOWrapper::SecureClientConfiguration()
{
    ...
    /*Connect to server*/
    connection_bio = BIO_new_connect(m_InterfaceConfig->DestinationIPandPort);
    ...
    /*Set hostname*/
    RetVal = SSL_set_tlsext_host_name(ssl,m_InterfaceConfig->SecureConfigParam.ExpectedPeerName.data());
    if(RetVal == 0)
    {
    	/* Close Connection */
    	CloseConnection();
    	
        throw std::runtime_error("R:Peername Set Err");
    }
    ...
    else
    {
        ...
        if (BIO_do_handshake(comm_bio) <= 0)
        {
            ...
            throw std::runtime_error("R:TLS handshake");
        }
    }
...
void BIOWrapper::CloseConnection()
{
	/*For non secure communication*/
	if(!m_InterfaceConfig->SecureConfigParam.IsEnable)
	{
		/*Free all BIO allocations*/
		if (comm_bio != nullptr) { 	BIO_free_all(comm_bio); }
	}
	...
	{	
		...
		/* sets the accept BIO close flag */
		BIO_set_close(bio,BIO_CLOSE);
		
		// TODO : Verify shutdown logic for all modes, currently verified for Secure / non-blocking
		// Perform graceful shutdown if no error has occurred.
		if (ssl != nullptr)
		{
			if(CommStatusResult >= STATUS_CONNECTION_DEFAULT)
			{
			...
			// clear allocated variable.
			SSL_free(ssl);
			SSL_CTX_free(ctx);
		}
		
		/* Free connection bio */
		if (connection_bio != nullptr) { BIO_free(connection_bio);}
...
/**************************************************************************//**
*
* \brief     - BIOWrapper : Destructor for safe disconnection.
