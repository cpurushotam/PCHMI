...
Item {
    ...
    property double ampMinVal               : TestGraphSettings.AmpAxisMinValue
    property double ampMaxVal               : TestGraphSettings.AmpAxisMaxValue

    property double powerMinVal             : TestGraphSettings.PowerAxisMinValue
    property double powerMaxVal             : TestGraphSettings.PowerAxisMaxValue

    property double freqMinVal              : TestGraphSettings.FreqAxisMinValue
    property double freqMaxVal              : TestGraphSettings.FreqAxisMaxValue

    property double phaseMinVal             : TestGraphSettings.PhaseAxisMinValue
    property double phaseMaxVal             : TestGraphSettings.PhaseAxisMaxValue

    property double currentMinVal           : TestGraphSettings.CurrentAxisMinValue
    property double currentMaxVal           : TestGraphSettings.CurrentAxisMaxValue
    ...
    **/
    function plotGraph()
    {
        ...
        if(isAmpLeftAxisVisible && isGraphEnable)
            TestGraphData.appendSamples(graphChartView.series(ampLeftPlotName), GraphAxisEnum.AMP_IDX);
        if(isPowerLeftAxisVisible && isGraphEnable)
            TestGraphData.appendSamples(graphChartView.series(powerLeftPlotName), GraphAxisEnum.POWER_IDX);
        if(isFreqLeftAxisVisible && isGraphEnable)
            TestGraphData.appendSamples(graphChartView.series(freqLeftPlotName), GraphAxisEnum.FREQ_IDX);
        if(isPhaseLeftAxisVisible && isGraphEnable)
            TestGraphData.appendSamples(graphChartView.series(phaseLeftPlotName), GraphAxisEnum.PHASE_IDX);
        if(isCurrentLeftAxisVisible && isGraphEnable)
            TestGraphData.appendSamples(graphChartView.series(currentLeftPlotName), GraphAxisEnum.CURRENT_IDX);

        /* Update the Min and Max values */
       /* var axisMinValues = TestGraphData.getAxisMinParameters();
        var axisMaxValues = TestGraphData.getAxisMaxParameters();
